...
@section('title')
    @lang('admin/server.breadcrumb_servers') — {{ $server->name }}: @lang('admin/server.page_manage_title')
@endsection

@section('content-header')
    <h1>{{ $server->name }}<small>@lang('admin/server.manage_server_description')</small></h1>
    <ol class="breadcrumb">
        ...
        <li class="active">@lang('admin/server.page_manage_title')</li>
    </ol>
@endsection
    ...
    <div class="row">
        <div class="col-sm-4">
            <div class="box box-danger">
                <div class="box-header with-border">
                    <h3 class="box-title">@lang('admin/server.reinstall_server_title')</h3>
                </div>
                <div class="box-body">
                    <p>{!! __('admin/server.reinstall_server_description') !!}</p>
                </div>
                <div class="box-footer">
                    @if($server->isInstalled())
                        <form action="{{ route('admin.servers.view.manage.reinstall', $server->id) }}" method="POST">
                            {!! csrf_field() !!}
                            <button type="submit" class="btn btn-danger">@lang('admin/server.reinstall_server_button')</button>
                        </form>
                    @else
                        <button class="btn btn-danger disabled">@lang('admin/server.server_must_install_properly')</button>
                    @endif
                </div>
            </div>
        </div>
        <div class="col-sm-4">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">@lang('admin/server.install_status_title')</h3>
                </div>
                <div class="box-body">
                    <p>@lang('admin/server.install_status_description')</p>
                </div>
                <div class="box-footer">
                    <form action="{{ route('admin.servers.view.manage.toggle', $server->id) }}" method="POST">
                        {!! csrf_field() !!}
                        <button type="submit" class="btn btn-primary">@lang('admin/server.toggle_install_status')</button>
                    </form>
                </div>
            </div>
        </div>
