    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">@lang('admin/server.base_information')</h3>
            </div>
            <form action="{{ route('admin.servers.view.details', $server->id) }}" method="POST">
                <div class="box-body">
                    <div class="form-group">
                        <label for="name" class="control-label">@lang('admin/server.server_name_label') <span class="field-required"></span></label>
                        <input type="text" name="name" value="{{ old('name', $server->name) }}" class="form-control" />
                        <p class="text-muted small">{!! __('admin/server.server_name_limits') !!}</p>
                    </div>
                    <div class="form-group">
                        <label for="external_id" class="control-label">@lang('admin/server.external_identifier')</label>
                        <input type="text" name="external_id" value="{{ old('external_id', $server->external_id) }}" class="form-control" />
                        <p class="text-muted small">@lang('admin/server.external_identifier_help')</p>
                    </div>
                    <div class="form-group">
                        <label for="pUserId" class="control-label">@lang('admin/server.server_owner_label') <span class="field-required"></span></label>
                        <select name="owner_id" class="form-control" id="pUserId">
                            <option value="{{ $server->owner_id }}" selected>{{ $server->user->email }}</option>
                        </select>
                        <p class="text-muted small">@lang('admin/server.server_owner_change_help')</p>
                    </div>
                    <div class="form-group">
                        <label for="description" class="control-label">@lang('admin/server.server_description_label')</label>
                        <textarea name="description" rows="3" class="form-control">{{ old('description', $server->description) }}</textarea>
                        <p class="text-muted small">@lang('admin/server.server_description_brief')</p>
                    </div>
                </div>
                <div class="box-footer">
                    {!! csrf_field() !!}
                    {!! method_field('PATCH') !!}
                    <input type="submit" class="btn btn-sm btn-primary" value="@lang('admin/server.update_details')" />
                </div>
            </form>
        </div>
    </div>
